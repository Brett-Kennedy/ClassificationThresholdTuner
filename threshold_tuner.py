...
-----
write up readme. 
- Include the content from create_plots notebook. 

2 types of viz for set threshold with multi-class: where threshold is over 0.5, and where under. 

option to plot by rank so can understand AUROC better. 

check the docstrings are correct

create pip install!

Show the team at work! 

give confidence for scores -- maybe. or maybe that's another project. If we have 5 classes, so 5 probabilities, and
take the highest that's over its threshold, how accurate? can just calibrate I think.

write medium article 
"""


class ClassificationThesholdTuner:
    def __init__(self):
        self.colors = ['lightseagreen', 'blue', 'olive', 'brown', 'yellow', 'purple', 'pink', 'grey',
    ...
    def print_stats_labels(self, y_true, target_classes, y_pred):
        """
    ...
    def print_stats_table(self, y_true, target_classes, y_pred_proba, num_ranges=10):
        """
        ...
        :param y_true: array. True labels for each record
        :param y_pred: array. Predicted labels for each record
        :param target_classes: array. Set of labels. Specified to allow displaying the positive class by name.
        :param num_ranges: The number of rows in the table displayed.
        :return: None
        """

        if len(target_classes) > 2:
            print("Currently available only for binary classification")
        ...
        # todo: handle missing ranges
        # todo: include cummulative values, maybe in a 2nd plot
    ...
    def print_stats_proba(self, y_true, target_classes, y_pred_proba, default_class=None, thresholds=None):
        ...
        :param y_true: array.
            True labels for each record
        :param target_classes: A list of the unique values. Must include all values in y_true, though may include
        ...
        :param y_pred_proba: If target_values contains 3 or more values, y_pred_proba must be a 2d matrix with a
        ...
        :param thresholds: For binary classification should be a float. For multiclass classification, should be an
        ...
        def two_classes():
            ...
            if len(true_labels) <= len(self.colors):
        ...
        def two_classes_horizontal(target_class_idx, y_true_orig, target_classes_orig, y_pred_proba_orig):
            """
            Called only in the multi-class case. This is called once for each class.

            :param target_class_idx:
            :return:
            """
            nonlocal thresholds
            ...
            true_labels = y_true_orig.unique()

            fig, ax = plt.subplots(ncols=3, figsize=(13, 3.5), gridspec_kw={'width_ratios': [1, 1, 1]})
            ...
            # todo: handle when there are more classes than colours defined
            for label_idx, label in enumerate(true_labels):
                d_sub = d[d['Y'] == label]
                if len(target_classes_orig) > 6:
                    sns.kdeplot(d_sub['Pred_Proba'], color=self.colors[label_idx], alpha=0.4, ax=ax[1])
                else:
                    sns.histplot(d_sub['Pred_Proba'], color=self.colors[label_idx], alpha=0.4, ax=ax[1])
            if target_classes_orig[target_class_idx] != default_class:
            ...
            swarm_dot_size = self.__get_swarmplot_dot_size(len(d_sample), y_true)
            # todo: colour the points red if wrong
            ...
            sns.swarmplot(data=d_sample, orient='h', x="Pred_Proba", y="Y", s=swarm_dot_size, palette=colors,
                          hue='colour_code', order=target_classes_orig, ax=ax[2])
            if target_classes_orig[target_class_idx] != default_class:
        ...
        def multi_class_simple_display():
            ...
            # Also plot with true prediction on the x-axis. # todo: explain gives sense of overall accuracy.
        ...
        def multi_class_with_default():
            ...
            for target_class_idx, target_class in enumerate(target_classes):
                ...
                two_classes_horizontal(target_class_idx, y_true_orig, target_classes_orig, y_pred_proba_orig)
        ...
        if len(target_classes) == 2:
            ...
            if default_class is None:
                ...
                multi_class_with_default()

    # todo: explain somewhere that red means wrong. otherwise the colour for that class
    def plot_by_threshold(self, y_true, target_classes, y_pred_proba, default_class=None, start=0.1, end=0.9, num_steps=9):
        ...
        def two_class():
            ...
            for threshold in tqdm(thresholds):
                ...
                # todo: also display precision, recall, f1 on both classes
                d_sample = self.__get_sample_df(d, target_classes)
        ...
        def multi_class():
            ...
            fig, ax = plt.subplots(nrows=num_plots, ncols=len(target_classes) + 1, sharex=False, sharey=True,
                                   ...
                                   gridspec_kw={'width_ratios': [1]*(len(target_classes) + 1)})
            ...
            for threshold in tqdm(thresholds):
                ...
                for target_class_idx, target_class in enumerate(target_classes):
                    ...
                    # todo: also display precision, recall, f1 on all classes
                    d_sample['colour_code'] = 'xxx'
                    colors = []
                    ...
                    sns.swarmplot(data=d_sample, orient='h', x=d[target_class], y="Y", size=swarm_dot_size,
    ...
    def get_predictions(target_classes, y_pred_proba, default_class, thresholds):
        ...
        if len(target_classes) == 2:
            # todo: if we get y_pred_proba as a 2D array where the length of each element is 1, convert to a 1d array
            # and remove checks for this.
